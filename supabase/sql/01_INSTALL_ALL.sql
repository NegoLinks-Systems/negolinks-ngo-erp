...
-- #############################################################################
-- ## STEP 007 — First-run setup, invitations and public donations
-- #############################################################################
...
BEGIN
  ...
  -- The first-run functions must exist, or nobody can create the first account.
  SELECT count(*) INTO missing_setup
  FROM (VALUES ('needs_bootstrap'), ('bootstrap_first_admin'), ('claim_invitation'),
               ('record_public_donation')) AS required(fn)
  WHERE NOT EXISTS (
    SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname = required.fn
  );
  ...
  IF missing_setup > 0 THEN
    RAISE EXCEPTION 'Installation incomplete: % first-run function(s) are missing.', missing_setup;
  END IF;
  ...
  RAISE NOTICE '   2. Open the app — it offers first-time setup while no';
  RAISE NOTICE '      organization exists. That first account is Super Admin.';
